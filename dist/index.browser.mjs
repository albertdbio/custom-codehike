...
import { extractAnnotations, highlight as highlight$1, LANG_NAMES, getThemeColors } from '@code-hike/lighter';
...
const CH_CODE_CONFIG_VAR_NAME = "chGlobalConfig";
/**
...
function toJSX(node, { type = "mdxJsxFlowElement", props, name, appendProps = false, addConfigProp = false, }) {
    ...
    // }
    if (addConfigProp) {
        node.attributes.push(toAttribute("globalConfig", CH_CODE_CONFIG_VAR_NAME, {
            type: "Identifier",
...
function extractLighterAnnotations(codeWithAnnotations, lang, annotationNames) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield extractAnnotations(codeWithAnnotations, warnIfUnknownLang(lang), annotationNames);
    });
}
function parseLighterAnnotations(annotations) {
    const focusList = [];
...
function warnIfUnknownLang(lang) {
    if (!LANG_NAMES.includes(lang)) {
        if (!warnings.has(lang)) {
            console.warn("[Code Hike warning]", `${lang} isn't a valid language, no syntax highlighting will be applied.`);
            warnings.add(lang);
        }
        return "text";
    }
...
function highlight({ code, lang, theme, }) {
    return __awaiter(this, void 0, void 0, function* () {
        const r = yield highlight$1(code, warnIfUnknownLang(lang), theme);
...
function splitCodeAndAnnotations(rawCode, lang, config) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        let { code, annotations } = yield extractLighterAnnotations(rawCode, lang, [
            ...annotationNames,
            "from",
            "focus",
        ]);
        ...
        if (fromAnnotations.length === 1) {
            const fromData = (_a = fromAnnotations[0].query) === null || _a === void 0 ? void 0 : _a.trim();
            const [codepath, range] = (fromData === null || fromData === void 0 ? void 0 : fromData.split(/\s+/)) || [];
            const externalFileContent = yield readFile(codepath, config.filepath, range);
            const result = yield extractLighterAnnotations(externalFileContent, lang, [...annotationNames, "focus"]);
            code = result.code;
        ...
        return Object.assign({ code }, parseLighterAnnotations(annotations));
    });
}
function readFile(externalCodePath, mdxFilePath, range) {
    return __awaiter(this, void 0, void 0, function* () {
...
function findLinkAnnotations(code) {
...
function mapFile({ node, index, parent }, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { code, annotations: commentAnnotations, focus: commentFocus, } = yield splitCodeAndAnnotations(node.value, lang, config);
        let highlightedCode = yield highlight({
            code,
            lang,
            theme,
        });
        ...
        return Object.assign(Object.assign({}, options), { code: highlightedCode, focus: mergeFocus(options.focus, commentFocus), name: options.name || "", annotations: [
                ...metaAnnotations,
                ...commentAnnotations,
            ] });
    });
...
function transformCode(nodeInfo, config) {
    return __awaiter(this, void 0, void 0, function* () {
        toJSX(nodeInfo.node, {
            name: "CH.Code",
            props: {
                editorStep: yield mapAnyCodeNode(nodeInfo, config),
            },
            appendProps: true,
            addConfigProp: true,
        });
...
function toGlobalConfig({ theme, staticMediaQuery, lineNumbers, showCopyButton, triggerPosition, }) {
    return {
...
function addConfig(tree, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const style = yield getStyle(theme, globalConfig.themeName);
